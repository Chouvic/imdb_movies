...
class MovieDataParse:
    ...
    def titles_mapping(self):
        ...
        return {
            slugify(item): {'url': None, 'abstract': None, 'title': item} for item in self.df['title'].to_list()
        }
    ...
    def save_required_columns_by_ratio(df):
        ...
        required_columns = [
            'title',
            'budget', 'year', 'revenue', 'vote_average', 'ratio', 'production_companies', 'url',
            'abstract'
        ]
        file_name = 'clean_result_data.csv'
        file_path = settings.DATASET_DIR + file_name
        df.to_csv(file_path, columns=required_columns)
        logger.info('Saved CSV with columns(%s) to %s.', ' '.join(required_columns), file_path)
